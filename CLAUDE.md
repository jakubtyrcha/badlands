...
- **Hero brain is Nim→WASM, the only hero brain** — run in a wasmtime host (`src/crates/brainhost`) behind the wire contract `game/src/brain_abi.h`; sources in `scripts/brains/nim/`, built to LFS-committed `.wasm` under `assets/brains/`. No mock/C++/noiser hero decision layer remains; a world with no wasm bytes loaded simply idles heroes. noiser itself lives on only for mapgen/texgen (`crates/noiser-bundle`, `third_party/noiser`, mapview/texgen tools) — a separate subsystem.
