...
**Impl**: PriorityFlood on final phase-0 bed → `h` in depressions; `route_flow`+`accumulate_drainage` (already linked) → D8 discharge → Manning depth per channel cell (reuse `channel_width_coeff`, pattern at 1783); fluxes start zero (safe from well-balanced start).
