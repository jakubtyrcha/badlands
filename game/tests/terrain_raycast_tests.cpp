...
// Owns a map and the lattice view of it. RaycastTerrain reads a lattice now --
// heights are all it ever needed -- and the view holds raw pointers into the
// map, so the two have to travel together.
struct Picked {
  MapData map;
  mutable TerrainLatticeStorage store;
  const TerrainLattice& operator*() const {
    store = map.Lattice();
    return store.View();
  }
};

Picked Pick(MapData m) { return Picked{std::move(m), {}}; }
...
TEST_CASE("MapData::HeightAt: bilinear, clamped at the edges") {
  ...
  const Picked m = Pick(RampX(kSize, kSize, kSlope));
...
TEST_CASE("RaycastTerrain: straight down onto flat ground") {
  const Picked m = Pick(Flat(64, 64, 5.0f));
  glm::vec3 hit;
...
TEST_CASE("RaycastTerrain: an angled ray lands ON the slope, not the plane") {
  ...
  const Picked m = Pick(RampX(64, 64, 1.0f));
...
TEST_CASE("RaycastTerrain: hit always lies on the sampled surface") {
  const Picked m = Pick(RampX(64, 64, 0.25f));
...
TEST_CASE("RaycastTerrain: an empty map is not a crash") {
  const Picked empty = Pick(MapData{});
  glm::vec3 hit;
