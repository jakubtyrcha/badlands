...
// The TreeOptions these fixtures shade with. Only the leaf fields matter --
// they drive the crown material's tint and the impostor's transmission.
TreeOptions FixtureOptions() {
  TreeOptions options;
  options.leaves.tint = glm::vec3(0.2f, 0.9f, 0.3f);
  options.leaves.transmission_strength = 0.6f;
  options.leaves.alpha_cutoff = 0.5f;
  return options;
}

// Assembles a ONE-LEVEL tree-shaped model from a hand-built trunk and crown,
// with the shipping tree materials and the same impostor spec
// BuildTreeFieldModel produces. One level means no thresholds -- the impostor
// the field builder appends is the only other level.
InstancedLodModel MakeTreeLikeModel(TexturedMeshResult trunk,
                                    TexturedMeshResult crown) {
  const TreeOptions options = FixtureOptions();
  InstancedLodModel model;
  model.submesh_materials = TreeSubmeshMaterials(options);
  model.levels.push_back({std::move(trunk), std::move(crown)});
  ...
  model.impostor.submeshes.push_back(ImpostorBakeSubmesh{
      ...
      .albedo = nullptr, .voxel_brightness = 0.0f});
  model.impostor.submeshes.push_back(ImpostorBakeSubmesh{
      .lod = 0, .submesh = kTreeCrownSubmesh, .tint = options.leaves.tint,
      .albedo = nullptr, .voxel_brightness = 1.0f});
  return model;
}
...
InstancedLodModel MakeSphereModel() {
  TexturedMeshResult trunk = GenerateSphereTexturedMesh(kTrunkRadius, 12);
  FlattenBrightness(trunk);
  AddIdentityIndices(trunk);
  ...
  return MakeTreeLikeModel(std::move(trunk), std::move(crown));
}
...
InstancedLodModel MakeStippleModel() {
...
SphereImpostor BuildImpostor(RenderGpu& g, InstancedLodModel model) {
  SphereImpostor out;
