...
bool MapViewView::Initialize(const RenderContext& ctx) {
  ...
  // Either load the map from rasters on disk or build it in-process. The
  // in-process path is the same generator --preview-image-only dumps, so the
  // rendered terrain and the preview PNGs can never disagree; a LOADED map is
  // by definition simulated elsewhere, so that guarantee does not apply to it.
  t = clock::now();
  if (!load_dir_.empty()) {
    std::string err;
