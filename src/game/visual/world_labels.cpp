...
std::vector<ResolvedLabel> WorldLabelPool::Resolve(const AnchorLookup& live_pos) const {
  ...
  for (size_t i = 0; i < labels_.size(); ++i) {
    ...
    // Stack rank = number of NEWER labels on the same anchor. The newest sits
    // lowest (rank 0, just above the head); each older one is pushed up a slot.
    int stack_rank = 0;
    for (size_t j = i + 1; j < labels_.size(); ++j) {
      if (labels_[j].anchor_slot == l.anchor_slot) {
        ++stack_rank;
      }
    }
    ...
    pos.y += l.base_offset + v.rise + static_cast<float>(stack_rank) * kStackGap;
